...
import { getDefaultModel as getGlobalDefaultModel, getServiceModel, getFallbackModel } from '../config/model-config.js';
...
/**
 * Get project root - simplified version compatible with existing utils
 */
...
/**
 * Create error context - simplified version
 */
function createErrorContext(module: string, method: string) {
  return {
    metadata: (data: any) => ({ build: () => ({ module, method, ...data }) }),
    build: () => ({ module, method })
  };
}
...
class ConfigurationError extends AppError {
  constructor(message: string, context?: any, details?: any) {
    super(message, context, details?.cause);
  }
}
...
export class OpenRouterConfigManager {
  ...
  private async _performInitialization(): Promise<void> {
    ...
    try {
      ...
      // Create OpenRouter configuration from environment - using centralized config
      this.config = {
        baseUrl: process.env.OPENROUTER_BASE_URL || 'https://openrouter.ai/api/v1',
        apiKey: process.env.OPENROUTER_API_KEY || '',
        defaultModel: getGlobalDefaultModel(),
      ...
      logger.info({
        ...
        researchModel: this.config.researchModel,
        mappingCount,
    ...
    if (!this.config) {
      try {
        ...
        logger.warn({
          err: error,
          operation,
          fallbackModel: getGlobalDefaultModel()
  ...
  private validateEnvironmentVariables(): EnvironmentValidationResult {
    ...
    if (!process.env.GEMINI_MODEL && !process.env.VIBE_DEFAULT_LLM_MODEL) {
      warnings.push('Neither GEMINI_MODEL nor VIBE_DEFAULT_LLM_MODEL set, using default');
    }
  ...
  validateConfiguration(): ConfigValidationResult {
    ...
    if (!this.config.defaultModel) {
      errors.push('Missing default model');
      suggestions.push('Set GEMINI_MODEL or VIBE_DEFAULT_LLM_MODEL environment variable');
    }

    if (!this.config.researchModel) {
    ...
    // Model validation - updated to use defaultModel
    if (this.config.defaultModel && !this.config.defaultModel.includes('gemini') && !this.config.defaultModel.includes('google')) {
      warnings.push('Default model does not appear to be a Google/Gemini model');
    }

    if (this.config.researchModel && !this.config.researchModel.includes('perplexity')) {
      warnings.push('RESEARCH_MODEL does not appear to be a Perplexity model');
    }
